...
// 引入OpenZeppelin库中的IERC721Receiver和IERC165接口
...
interface IERC404 is IERC165 {
  ...
  function totalSupply20() external view returns (uint256);
  function erc20TotalSupply() external view returns (uint256);
  function erc721TotalSupply() external view returns (uint256);
  function balanceOf20(address owner_) external view returns (uint256);
...
abstract contract ERC404 is IERC404 {
  ...
  ///      totalSupply721 id, monotonically increasing to ensure accurate ownership
  /// erc721总供应量
  uint256 public totalSupply721;
  ...
  mapping(address => uint256) public balanceOf20;
  ...
  function erc20BalanceOf(
    ...
    return balanceOf20[owner_];
  }

  function erc20TotalSupply() public view virtual returns (uint256) {
    return totalSupply20;
  }

  function erc721TotalSupply() public view virtual returns (uint256) {
    return totalSupply721;
  }
  ...
  ///         necessarily have to be totalSupply721 yet.
  ...
  function _transferERC20(
    ...
    if (from_ == address(0)) {
      totalSupply20 += value_;
    } else {
      // Deduct value from sender's balance.
      balanceOf20[from_] -= value_;
    }

    // Update the recipient's balance.
    // Can be unchecked because on mint, adding to totalSupply20 is checked, and on transfer balance deduction is checked.
    unchecked {
      balanceOf20[to_] += value_;
    }
  ...
  function _transferERC20WithERC721(
    ...
    if (isFromERC721TransferExempt && isToERC721TransferExempt) {
      ...
      //         from the bank/totalSupply721 for any whole number increase in their balance.
      // Only cares about whole number increments.
      uint256 tokensToRetrieveOrMint = (balanceOf20[to_] / units) -
      ...
      //         receive ERC-721s from the bank/totalSupply721.
      // Only cares about whole number increments.
      uint256 tokensToWithdrawAndStore = (erc20BalanceOfSenderBefore / units) -
        (balanceOf20[from_] / units);
  ...
  function _mintERC20(address to_, uint256 value_) internal virtual {
    ...
    if (totalSupply20 + value_ > ID_ENCODING_PREFIX) {
  ...
  // ERC-721铸币和从银行检索的内部函数。
  // 这个函数将允许新的ERC-721s的铸造，直到总分数供应。它将首先尝试从银行取出，如果银行是空的，它将铸造一个新的令牌
  // 不处理ERC-721豁免。
  ...
  function _retrieveOrMintERC721(address to_) internal virtual {
    ...
    if (!_storedERC721Ids.empty()) {
      ...
      if (totalSupply721 == type(uint256).max) {
        revert MintLimitReached();
      }

      id = ID_ENCODING_PREFIX + totalSupply721;
    }
...
contract My404 is Ownable, ERC404, ERC404MerkleClaim {

  constructor() ERC404("TEST", "test", 18) Ownable(msg.sender) {
    // Do not mint the ERC721s to the initial owner, as it's a waste of gas.
    _setERC721TransferExempt(msg.sender, true);
    _mintERC20(msg.sender, 100 * units);
  }
